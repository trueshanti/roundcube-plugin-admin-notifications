...
class admin_notifications extends rcube_plugin
{
  ...
  function notifications_data($args)
  {
    if ($args['section'] == 'notifications') {
      ...
      $args['blocks']['create_notif'] = array(
          'options' => array(),
          'name'    => rcube::Q($this->gettext('notifications_create'))
      );
      $args['blocks']['list_notif'] = array(
          'options' => array(),
          'name' => rcube::Q($this->gettext('notifications')),
          'cols' => 1
      );
      ...
      $args['blocks']['create_notif']['options'][] = array(
          ...
          'content' => html::tag('textarea', array(
              'name'       => '_message',
              'title'      => rcube::Q($this->gettext('notification_message')),
      ...
      $args['blocks']['create_notif']['options'][] = array(
          'content' => $this->api->output->button(array(
              'type' => 'input',
  ...
  function save_notification($attrib)
  {
    if ($attrib['section'] == "notifications" and !$attrib['abort']){
      ...
      if (is_numeric($id) and !$this->notification_exists($id)) {
        $attrib['abort'] = true;
        return $attrib;
      }
      if (!is_numeric($id)) {
  ...
  function enable_notifications($args)
  {
    ...
    if (!$last_id or $last_id < 1){
      ...
      if (empty($prefs[$this->pref_name]) or $prefs[$this->pref_name]['last'] < $last_id) {
  ...
  function render_page($p)
  {
    if ($_SESSION[$this->sess_name]) {
      $prefs = $this->rc->user->get_prefs();
      $last = !empty($prefs[$this->pref_name]) ? $prefs[$this->pref_name]['last'] : 0;
